...
void DrawBitLine16BPP0(int x, int y, int xsize, const LCD_PIXELINDEX *pTrans)
{
		u16 w;
		XY_Coordinate(x,y);
...
void DrawBitLine8BPP0(int x, int y, int xsize, U8 const *pTrans)
{
		u16 w;
		XY_Coordinate(x,y);
		WriteCommand(0x02);		//MRWC  REG[02h] -- Memory Read/Write Command
...
//				}
				DrawBitLine16BPP0(x,y,xsize,pTrans + (*p));
			}
			else
			{
//				for (; xsize > 0; xsize--, x++, p++)
...
#define yPhys y
  ...
  if(xPhys <= 480 && yPhys <= 272)
	{
	XY_Coordinate(xPhys,yPhys);
...
//			LCD_L0_XorPixel(x0, y);
//		}
		PixelIndex=LCD_L0_GetPixelIndex(x0, y);
		lcd_hline(x0,y,x1,LCD_NUM_COLORS - PixelIndex - 1);
	}
	else

        lcd_hline(x0,y,x1,LCD_COLORINDEX);
#endif
}
...
//			LCD_L0_XorPixel(x, y0);
//		}
		PixelIndex=LCD_L0_GetPixelIndex(x, y0);
		lcd_hline(x,y0,y1,LCD_NUM_COLORS - PixelIndex - 1);
	}
	else
        lcd_vline(x,y0,y1,LCD_COLORINDEX);
#endif																 
}
...
#else
	LcdFillRec(x0, y0, x1,y1,1, LCD_COLORINDEX);
#endif
}
