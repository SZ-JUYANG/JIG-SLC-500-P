...
void LCD_L0_SetPixelIndex(int x, int y, int PixelIndex)
{
	...
	int xPhys = LOG2PHYS_X(x, y);  //逻辑到物理，这里没有交换
...
#define yPhys y
    ...
    if(xPhys <= 480 && yPhys <= 272)
	{
		...
		WriteData((unsigned short)PixelIndex);
	}	
}
   ...
   const LCD_PIXELINDEX *pTrans)
{
	...
	LCD_PIXELINDEX pixel;
	const U16 * p;
	p=(const U16 *)pData;//这里要强制类型转换
	for (; ysize>0;ysize--,y++)
	{
		x_size=xsize;
		x=x0;
		if ((GUI_Context.DrawMode & LCD_DRAWMODE_TRANS) == 0) //如果为
		{
			if (pTrans)
			{
				for (; x_size > 0; x_size--, x++, p++)
				{
					pixel = *p;
					LCD_L0_SetPixelIndex(x, y, *(pTrans + pixel));
				}
