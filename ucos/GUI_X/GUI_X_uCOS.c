...
{ 
    ...
    OSSemPend(DispSem, 0, &err);			//上锁，等待信号量
}

void  GUI_X_Unlock (void)
{ 
    OSSemPost(DispSem);						//解锁，提交信号量
}
...
void GUI_X_WaitEvent (void) 
{
    ...
    (void)OSMboxPend(EventMbox, 0, &err);	  //等待消息邮箱
}

void GUI_X_SignalEvent (void) 
{
    (void)OSMboxPost(EventMbox, (void *)1);	  //向消息邮箱发消息
}
