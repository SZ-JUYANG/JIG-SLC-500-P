...
/                   without write.
/
/ Apr 01,'08 R0.06  Added f_forward(), f_fputc(), f_fputs(), f_fprintf() and f_fgets().
...
void get_fileinfo (	/* No return code */
	...
	u8 *dir	/* Ptr to the directory entry */
)
{
	u8 n, c, a;
	char *p;
...
)
{
	...
	for (;;) {
		...
		if (c <= '?') break;				/* Reject : ; < = > ? */
		if (!(a & 1)) {	/* These checks are not applied to S-JIS 2nd u8 */
			if (c == '|') break;			/* Reject | */
			if (c >= '[' && c <= ']') break;/* Reject [ \ ] */
...
)
{
	...
	clust1=clust;
	clust=create_chain(dj->clust);
	if (clust1 == 0 || !clust )return FR_DENIED;
...
)
{
	...
	res = auto_mount(&path, (u8)(mode & (FA_WRITE|FA_CREATE_ALWAYS|FA_OPEN_ALWAYS|FA_CREATE_NEW)));
#else
	mode &= FA_READ;
...
FRESULT f_read (
	...
	UINT btr,		/* Number of u8s to read */
	UINT *br		/* Pointer to number of u8s read */
)
{
	FRESULT res;
	DWORD sect, remain;
	...
	*br = 0;
	...
	if (btr > remain) btr = (UINT)remain;			/* Truncate btr by remaining u8s */
	...
	for ( ;  btr;									/* Repeat until all data transferred */
		...
		if ((fp->fptr % 512U) == 0) {				/* On the sector boundary? */
			...
			cc = btr / 512U;						/* When remaining u8s >= sector size, */
			if (cc) {								/* Read maximum contiguous sectors directly */
				if (fp->csect + cc > fp->fs->csize)	/* Clip at cluster boundary */
					cc = fp->fs->csize - fp->csect;
				if (disk_read(0, rbuff, sect, (u8)cc) != RES_OK)
					goto fr_error;
				fp->csect += (u8)cc;				/* Next sector address in the cluster */
				rcnt = 512U * cc;					/* Number of u8s transferred */
				continue;
			}
...
FRESULT f_write (
	...
	UINT btw,			/* Number of u8s to write */
	UINT *bw			/* Pointer to number of u8s written */
)
{
	FRESULT res;
	DWORD sect;
	...
	*bw = 0;
	...
	for ( ;  btw;									/* Repeat until all data transferred */
		...
		if ((fp->fptr % 512U) == 0) {				/* On the sector boundary? */
			...
			sect = clust2sect(fp->curr_clust) + fp->csect;	/* Get current sector */
			cc = btw / 512U;						/* When remaining u8s >= sector size, */
			if (cc) {								/* Write maximum contiguous sectors directly */
				if (fp->csect + cc > fp->fs->csize)	/* Clip at cluster boundary */
					cc = fp->fs->csize - fp->csect;
				if (disk_write(0, wbuff, sect, (u8)cc) != RES_OK)
					goto fw_error;
				fp->csect += (u8)cc;				/* Next sector address in the cluster */
				wcnt = 512U * cc;					/* Number of u8s transferred */
				continue;
			}
...
)
{
	...
	if (ofs > 0) {
		csize = (DWORD)fp->fs->csize * 512U;		/* Cluster size (u8) */
		if (ifptr > 0 &&
...
FRESULT f_getfree (
	char *drv,	/* Pointer to the logical drive number (root dir) */
...
FRESULT f_forward (
	FIL *fp, 						/* Pointer to the file object */
	UINT (*func)(u8*,UINT),	/* Pointer to the streaming function */
	UINT btr,						/* Number of u8s to forward */
	UINT *br						/* Pointer to number of u8s forwarded */
)
{
	FRESULT res;
	DWORD remain;
...
)
{
	...
	while (i < len - 1) {			/* Read u8s until buffer gets filled */
		f_read(fil, p, 1, &rc);
	...
	*p = 0;
	return i ? buff : 0;			/* When no data read (f_EOF or error), return with error. */
}
...
)
{
...
#if _USE_STRFUNC >= 2
	if (chr == '\n') f_fputc ('\r', fil);	/* LF -> CRLF conversion */
#endif
	...
	c = (char)chr;
	f_write(fil, &c, 1, &bw);	/* Write a u8 to the file */
	return bw ? chr : f_EOF;		/* Return the resulut */
}
...
)
{
	...
	for (n = 0; *str; str++, n++) {
		if (f_fputc(*str, fil) == f_EOF) return f_EOF;
	}
	return n;
}
...
int f_fprintf (
	FIL* fil,			/* Pointer to the file object */
	char* str,	/* Pointer to the format string */
	...					/* Optional arguments... */
)
{
	va_list arp;	  //  typedef struct __va_list { void *__ap; } va_list;
	UCHAR c, f, r;	  
	ULONG val;
	char s[16];
	int i, w, res, cc;
	...
	for (cc = res = 0; cc != f_EOF; res += cc) {
		...
		if (c == 's') {				/* Type is string */
			cc = f_fputs(va_arg(arp, char*), fil);
			continue;
		}
		if (c == 'c') {				/* Type is character */
			cc1=cc1;
			cc = f_fputc(va_arg(arp, int), fil);
			if (cc != f_EOF) cc = 1;
			continue;
		}
		r = 0;
