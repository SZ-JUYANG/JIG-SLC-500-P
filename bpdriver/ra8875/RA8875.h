...
//#define LCD_ShowString(p1,p2,p3) LcdPrintStr(p3,p1,p2,RED,WHITE,1)
//#define LCD_DrawLine(x1,y1,x2,y2)  lcd_line(x1,y1,x2,y2,0x0000)
#define Draw_Circle(x,y,size) Circle_Coordinate_Radius(x,y,size)
...
//-----------------------------------------------------------------------------------------------
	...
	#define LCD_COMM_ADD		*((vu16 *)0X60020000)          //命令或状态地址
	#define LCD_DATA_ADD		*((vu16 *)0X60000000)		   //数据读写地址

	#define ReadData()  (LCD_DATA_ADD)	   //数据读取
	#define ReadCOMM()  (LCD_COMM_ADD)	   //状态读取
	...
	#define writereg(reg,data) WriteCommand(reg);WriteData(data)


//STATUS 
void Writereg(u8,u8);//写寄存器
void Chk_Busy(void);
...
//REG[2Eh]
void FontSize(u8 font);
void FontSizeGaotong(u8 font);
void FontSizeSpiflash(u8 font);
